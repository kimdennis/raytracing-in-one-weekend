...
public:
    ...
    __device__ static vec3 random(curandState* rand_state, double min, double max) {
        return vec3(random_double(rand_state, min, max), random_double(rand_state, min, max), random_double(rand_state, min, max));
    }
...
__device__ __host__ inline vec3 operator/(vec3 v, double t) {
    return (1/t) * v;
}
